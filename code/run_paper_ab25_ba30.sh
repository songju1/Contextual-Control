...
# ============================================================
# Paper runs: AB25 and BA30, seeds 0..9
# model=I, wrong_goal_penalty=0.01, steps=300000
# Then evaluate and aggregate summaries.
# ============================================================
...
echo "============================================================"
echo "3) EVAL: AB25 latest 10 runs"
echo "============================================================"
for RUN in $(ls -dt runs/*_modelI_d32_a0.1_*_orderAB_switch25_*_nenv4* | head -n 10); do
...
echo "============================================================"
echo "4) EVAL: BA30 latest 10 runs"
echo "============================================================"
for RUN in $(ls -dt runs/*_modelI_d32_a0.1_*_orderBA_switch30_*_nenv4* | head -n 10); do
  python eval.py --runs_dir "$RUN" --episodes "${EVAL_EPISODES}" --deterministic \
    --eval_layout "${LAYOUT}" --eval_episode_len "${EP_LEN}" \
...
echo "============================================================"
echo "5) AGGREGATE: AB25"
echo "============================================================"
OUT_AB="paper_summaries/combined_summary_AB25_I_phase1only001_10seeds.csv"
echo "run,$(head -n 1 $(ls -dt runs/*_modelI_d32_a0.1_*_orderAB_switch25_*_nenv4* | head -n 1)/summary.csv)" > "${OUT_AB}"
for RUN in $(ls -dt runs/*_modelI_d32_a0.1_*_orderAB_switch25_*_nenv4* | head -n 10); do
...
echo "============================================================"
echo "6) AGGREGATE: BA30"
echo "============================================================"
OUT_BA="paper_summaries/combined_summary_BA30_I_phase1only001_10seeds.csv"
echo "run,$(head -n 1 $(ls -dt runs/*_modelI_d32_a0.1_*_orderBA_switch30_*_nenv4* | head -n 1)/summary.csv)" > "${OUT_BA}"
for RUN in $(ls -dt runs/*_modelI_d32_a0.1_*_orderBA_switch30_*_nenv4* | head -n 10); do
...
echo "============================================================"
echo "7) AGGREGATE: AB25 + BA30 combined"
echo "============================================================"
OUT_ALL="paper_summaries/combined_summary_AB25_BA30_I_phase1only001_20runs.csv"
